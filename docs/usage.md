...
This page is for operating tasks after the Gateway is paired. It describes what the Android app actually does. API calls that have no button are named as API-only.
...
Open **Workspaces**, pick a paired host, and open an allowed directory. The browser lists child directories. It hides dot-directories and does not run Git or other project inspection. Tap **Create task here**.

The app sends `POST /api/v1/sessions` with the host id and directory only. The Gateway starts `omp --mode rpc-ui` in that directory and leaves the task idle until you send a prompt. The title comes from the directory name until a prompt supplies one.

You cannot point the app at an OMP process you already started in a terminal. PinkCollab manages tasks its Gateway starts.
...
Tasks from every paired host appear in one pager, newest first. A page shows the title, status, and host name. Opening a task loads its timeline.
...
| `failed` | Failed | The turn or process failed. If the runtime is gone, you cannot prompt this task. |
| `stopped` | Stopped | You stopped the process, or it exited outside a completed or failed run. No further commands. |
| `offline` | Offline | The Gateway restarted while this task was live. The process was not restored. |
...
Live replies and tool activity update while the WebSocket is connected. The phone does not keep a private copy of the transcript. Force-closing the app drops the in-memory view; reopening it loads a snapshot and, when needed, the task detail again.

## Add instructions

Type in the composer and tap **Send**. The button label does not change. The placeholder does:

- empty task: "What should OMP do?"
...
Steering is useful only while a turn is running. If the task has already completed or gone idle, you are starting another turn, not nudging the one that finished.

## Answer a question

An attention card appears only when this task has a pending input request. The first-task example will not always produce one.
...
After a successful answer the task returns to running. A stale or mismatched request id is rejected; answer the card that is on screen, or reopen the task to refresh it.
...
**Interrupt** is enabled while the status is `running` or `needs_input` and the runtime is attached. There is no confirmation dialog. It sends OMP `abort`, moves the task to idle (the app label is Paused), clears a pending question, and leaves the process running. You can send another prompt.

**Stop** is enabled whenever the runtime is attached. The app asks you to confirm. It closes OMP's stdin and terminates the process if it has not exited after three seconds. The task becomes `stopped`, the runtime is detached, and Model, Interrupt, Stop, and Send are disabled. You cannot resume that process. The transcript stays visible.

A Gateway shutdown also stops the OMP processes that Gateway started. That is a host-side stop, not an Interrupt.

## Several hosts

Pair each computer separately. Android lists their tasks in one pager and keeps a connection per host. A host card shows whether that connection is online, reconnecting, offline, or needs pairing again.
...
If the process has exited, the runtime is detached and the composer stays disabled. Create a new task to continue in that project. The old transcript remains readable when the session file is still on the host.

`max_sessions` counts a starting task and every task whose OMP process is still alive. A completed task with a live process still counts. A stopped, failed, or completed task whose process has exited does not. The default limit is 8. The authoritative rule is in [reference](reference.md#max_sessions).

## After a disconnect or restart

These are not the same event.

**The phone loses the network, or you leave the app.** The OMP process keeps running on the host. When the app returns to the foreground, or the network comes back, it reconnects, takes a new snapshot, and reloads an open task. There is no replay of events missed while disconnected, and no system notification that a question arrived. Look at the task.

**The Gateway process restarts.** Running OMP processes are not reattached. Tasks that were `starting`, `running`, `needs_input`, or `idle` become `offline`, with `runtimeAttached` false. `completed`, `failed`, and `stopped` keep those statuses, also detached. Pairings and task metadata remain. History is read on demand from OMP's session file for the current branch, including tool calls and results that file still holds, up to the same 500-item retention as the live timeline. That is not the live event stream. Message deltas, sequence numbers, and anything already trimmed are gone. If the session file is missing, the detail can be empty.

**The host sleeps or shuts down.** PinkCollab does not keep a task running across that. Sleep suspends the machine's processes; shutdown ends them. On the next Gateway start, a task that was live shows as offline. Do not read "history is still there" as "the agent kept working."
...
- Delete a task. `DELETE /api/v1/sessions/:id` exists and removes Gateway metadata only after the runtime is gone. It never deletes OMP's session files. The app has no delete control.
